...
SEGMENT_COLOURS = {
    ...
    "Enterprise": "#2563eb",
    "Midmarket": "#3b82f6",
    "Small Business": "#60a5fa",
    "Channel Partners": "#93c5fd",
}
...
COUNTRY_COLOURS = {
    ...
    "Germany": "#f59e0b",
    "Mexico": "#fbbf24",
    "United States of America": "#fde68a",
}

PRODUCT_COLOUR = "#2563eb"
...
st.plotly_chart(style_chart(time_chart, showlegend=True), use_container_width=True)
...
left_col.plotly_chart(style_chart(product_chart), use_container_width=True)
...
right_col.plotly_chart(style_chart(product_margin_chart), use_container_width=True)
...
segment_chart = px.bar(
    segment_performance.sort_values("Profit"),
    x="Profit",
    y="Segment",
    orientation="h",
    color="Segment",
...
left_col.plotly_chart(style_chart(segment_chart), use_container_width=True)

segment_margin_chart = px.bar(
    segment_performance.sort_values("Profit Margin"),
    x="Profit Margin",
    y="Segment",
    orientation="h",
    color="Segment",
...
right_col.plotly_chart(style_chart(segment_margin_chart), use_container_width=True)
...
country_performance["Profit Margin"] = (
    country_performance["Profit"] / country_performance["Sales"]
)

left_col, right_col = st.columns(2)

country_chart = px.bar(
    country_performance.sort_values("Profit"),
    x="Profit",
    y="Country",
    orientation="h",
    color="Country",
...
left_col.plotly_chart(style_chart(country_chart), use_container_width=True)
...
right_col.plotly_chart(style_chart(discount_chart, showlegend=True), use_container_width=True)
...
discount_band_chart = px.bar(
    discount_band_summary.sort_values("Profit Margin"),
    x="Profit Margin",
    y="Discount Band",
    orientation="h",
...
st.plotly_chart(style_chart(discount_band_chart), use_container_width=True)
